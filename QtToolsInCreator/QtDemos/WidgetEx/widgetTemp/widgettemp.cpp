...
WidgetTemp::WidgetTemp(QWidget *parent) : QWidget(parent)
{
    init();
}

void WidgetTemp::setToolBar(QWidget *toolBar)
{
    m_toolbar = toolBar;
    m_toolbar->installEventFilter(this);
}

void WidgetTemp::init()
{
 ...
 * @param event
 */
void WidgetTemp::thisEvent(QObject *watched, QEvent *event)
{
    if(watched != this)
    {
        return;
...
void WidgetTemp::toolBarEvent(QObject *watched, QEvent *event)
{
    Q_UNUSED(event)
 ...
 * @return
 */
bool WidgetTemp::winMouseEvent(MSG *msg, long *result)
{
    // 获取鼠标坐标
    ...
    // 实现标题栏功能
    if (m_toolbar
     && m_toolbar->rect().contains(pos)  // 如果鼠标在标题栏中移动则进行处理
     && !m_toolbar->childAt(pos))        // 如果当前鼠标位置没有子控件（如标题栏上的按键）则进行移动窗口处理
    {
...
bool WidgetTemp::eventFilter(QObject *watched, QEvent *event)
{
    thisEvent(watched, event);
    toolBarEvent(watched, event);
    return QWidget::eventFilter(watched, event);
}

bool WidgetTemp::nativeEvent(const QByteArray &eventType, void *message, long *result)
{
#ifdef Q_OS_WIN
